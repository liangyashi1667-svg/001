...
function renderActions(actions) {
  if (!actions?.length) return `<p class="muted">暂无行动项</p>`;
  return `<ul class="actions">${actions
    .map((a) => {
      const mark = a.done ? "done" : "open";
      const meta = [a.owner, a.due ? `截止 ${a.due}` : null, a.meeting]
        .filter(Boolean)
        .join(" · ");
      return `<li class="${mark}"><span class="tick">${a.done ? "✓" : "○"}</span><div><p>${escapeHtml(a.text)}</p>${meta ? `<small>${escapeHtml(meta)}</small>` : ""}</div></li>`;
    })
    .join("")}</ul>`;
}
  ...
  const meetingCards = meetings
    ...
    .map((m, idx) => {
      ...
      <article class="meeting" id="m-${idx}">
        <header>
          <div>
            <p class="eyebrow">${escapeHtml(formatDate(m.date))} · ${escapeHtml(m.phase)}</p>
    ...
    .map((t) => {
      ...
      return `
        <li>
          <div class="dot"></div>
          <div>
            <p class="t-date">${escapeHtml(formatDate(t.date))}</p>
            <p class="t-title">${escapeHtml(t.title)}</p>
            <p class="t-meta">${escapeHtml(t.phase)}${t.progressPercent != null ? ` · ${t.progressPercent}%` : ""} ${delta}</p>
          </div>
        </li>`;
    })
    .join("");
...
<body>
  <div class="wrap">
    ...
    <section class="stats">
      ...
      <div class="stat"><span>已完成待办</span><strong>${project.doneActionCount}</strong></div>
    </section>
    ...
    <section class="panel">
      <h2>优先行动项</h2>
      ${renderActions(project.priorityActions)}
    </section>

    <section>
      <div class="panel" style="margin-bottom:0">
        <h2>每次会议纪要总结</h2>
        <p class="muted" style="margin:0">按会议时间顺序展开，含决议、风险与行动项。</p>
      </div>
      ${meetingCards}
    </section>
